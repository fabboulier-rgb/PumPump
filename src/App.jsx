...
export default function App() {
  ...
  // NOUVEAU STATE : Gère l'affichage du menu Excel
  ...
  return (
    <div className="min-h-screen bg-orange-50 dark:bg-indigo-950 flex flex-col font-sans text-slate-700 dark:text-slate-200 relative overflow-x-hidden transition-colors duration-500">
      ...
      {/* NOUVEAU SYSTÈME DE MENU : ONGLETS EXCEL PLIABLES */}
      {isMenuOpen ? (
        <div className="fixed bottom-0 left-0 right-0 bg-slate-200 dark:bg-slate-950 border-t border-slate-300 dark:border-slate-800 z-50 flex items-end pr-2 pt-2 shadow-[0_-4px_10px_rgba(0,0,0,0.1)]">
          
          {/* Onglets style Excel */}
          <div className="flex gap-1 px-2">
            <button 
              onClick={() => setActiveTab('pump')}
              className={`px-4 py-2 rounded-t-lg font-bold text-sm border border-b-0 transition-colors ${activeTab === 'pump' ? 'bg-white dark:bg-slate-900 text-rose-500 border-slate-300 dark:border-slate-800' : 'bg-slate-300 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 border-transparent hover:bg-slate-100 dark:hover:bg-slate-800'}`}
            >
              {t.tabPump}
            </button>
            <button 
              onClick={() => setActiveTab('feed')}
              className={`px-4 py-2 rounded-t-lg font-bold text-sm border border-b-0 transition-colors ${activeTab === 'feed' ? 'bg-white dark:bg-slate-900 text-blue-500 border-slate-300 dark:border-slate-800' : 'bg-slate-300 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 border-transparent hover:bg-slate-100 dark:hover:bg-slate-800'}`}
            >
              {t.tabFeed}
            </button>
          </div>

          {/* Mini-Sélecteur Thème */}
          <div className="mb-1 ml-auto flex bg-white dark:bg-slate-800 p-1 rounded-md shadow-sm border border-slate-300 dark:border-slate-700">
            <button onClick={() => { if (darkMode) toggleDarkMode(); }} className={`px-2 text-xs ${!darkMode ? 'opacity-100' : 'opacity-40'}`}>☀️</button>
            <button onClick={() => { if (!darkMode) toggleDarkMode(); }} className={`px-2 text-xs ${darkMode ? 'opacity-100' : 'opacity-40'}`}>🌙</button>
          </div>

          {/* Bouton pour cacher (Fermer) */}
          <button onClick={() => setIsMenuOpen(false)} className="mb-1 ml-2 px-3 py-1 bg-slate-300 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-md font-bold text-xs hover:bg-slate-400 dark:hover:bg-slate-700 transition-colors">
            ▼
          </button>
        </div>
      ) : (
        /* PETIT BOUTON POUR AFFICHER LE MENU (Mode caché) */
        <button 
          onClick={() => setIsMenuOpen(true)}
          className="fixed bottom-4 right-4 z-50 bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-4 py-2 rounded-full shadow-lg border border-slate-200 dark:border-slate-700 font-bold text-sm flex items-center gap-2 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all"
        >
          ▲ Menu
        </button>
      )}
